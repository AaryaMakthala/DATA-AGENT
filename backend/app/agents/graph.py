"""LangGraph workflow (CLAUDE.md §5).

Full node sequence: START -> Profiler Node -> LLM Analysis Node -> Cleaning Plan
Node -> Python Cleaning Node -> Visualization Node -> ML Recommendation Node -> END.
...
from app.tools.ml_recommender import MLRecommenderError, recommend_algorithms
...
def python_cleaning_node(state: AnalystState) -> dict:
    ...
    try:
        cleaned_file = clean_csv(state["file_path"], state.get("cleaning_plan"), file_id)
    except CleanerError as exc:
        logger.error("Graph: Python cleaning node failed: %s", exc)
        raise
    return {"cleaned_file": cleaned_file}


def visualization_node(state: AnalystState) -> dict:
    """Generate charts from the cleaned dataset."""
    logger.info("Graph: running visualization node")
    file_id = state.get("file_id") or uuid.uuid4().hex
    try:
        charts = generate_charts(state["cleaned_file"], file_id)
...
def ml_recommendation_node(state: AnalystState) -> dict:
    ...
    try:
        ...
        recommendations = recommend_algorithms(state["cleaned_file"], cleaned_profile)
...
def build_graph():
    ...
    graph.add_edge("profiler", "llm_analysis")
